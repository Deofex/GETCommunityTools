...
class TelegramBot():
    ...
    def sendphoto(
            self, chatid, photourl, caption, formatstyle="HTML"):
        method = 'sendPhoto'
        parameters = {
            "chat_id": chatid,
            "photo": photourl,
        ...
        getphotourl = self.createtelegramurl(method, parameters=parameters)
        logger.debug("Send photo via url: {}".format(getphotourl))
        results = get_json_from_url(getphotourl)
        return results

    def sendphotoall(self, photo, caption):
        tgids = self.db.get_tgchannels()
        for tgid in tgids:
            chatid = tgid['tgid']
            logger.debug("Send photo: {} with message: {} to chatid: {}".format(
                photo, caption, chatid))
            while True:
                sendphotoresult = self.sendphoto(chatid, photo, caption)
                if sendphotoresult['ok'] == False:
                    if sendphotoresult['error_code'] == 400:
                        logger.info('Sending photo failed:: {}'.format(photo))
                        raise Exception(sendphotoresult['error_code'],
                        sendphotoresult['description'])
                    elif sendphotoresult['error_code'] == 429:
                        logger.info('Rate request exceeded, waiting 60 sec')
                        time.sleep(60)
                    else:
                        break
